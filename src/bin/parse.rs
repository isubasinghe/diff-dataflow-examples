...
fn main() {
    ...
    let source = std::env::args().skip(2).next().unwrap();
    ...
    let mysource = source.to_owned();
    let file = File::open(source).expect(&format!("Failed to open source file {}", mysource));

    let input = BufReader::new(file);

    for line in input.lines().map(|x| x.unwrap()).filter(|x| !x.starts_with('#')) {
